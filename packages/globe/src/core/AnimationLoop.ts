...
import { useStore } from "../store/useGlobeStore";
import {
...
// â”€â”€ DOD: Pre-sorted entity buckets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function createUpdateLoop(
    ...
    return () => {
        ...
        // Pass 1: Dynamic entities â€” every frame
        ...
        // Pass 2: Static entities â€” cull frames only (unless interactive)
...
import { renderEntitiesChunked } from "./EntityRenderer";

let removePostUpdateListener: (() => void) | undefined;
let unsubscribeDataBus: (() => void) | undefined;
const existingMap = new Map<string, AnimatableItem>();
const animatablesRef = { current: [] as AnimatableItem[] };
const hoveredEntityIdRef = { current: null as string | null };

export function startAnimationLoop(viewer: CesiumViewer) {
    if (removePostUpdateListener) return;

    unsubscribeDataBus = dataBus.on("dataUpdated", async ({ entities }: any) => {
        // Base64 Data URIs for the icons
        const FLIGHT_ICON = 'data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2216%22%20height%3D%2216%22%20viewBox%3D%220%200%2016%2016%22%3E%3Cpolygon%20points%3D%228%2C2%2014%2C14%202%2C14%22%20fill%3D%22%230088ff%22%20stroke%3D%22%23ffffff%22%20stroke-width%3D%221%22%2F%3E%3C%2Fsvg%3E';
        const SHIP_ICON = 'data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2216%22%20height%3D%2216%22%20viewBox%3D%220%200%2016%2016%22%3E%3Cpolygon%20points%3D%228%2C2%2014%2C14%202%2C14%22%20fill%3D%22%23ff2222%22%20stroke%3D%22%23ffffff%22%20stroke-width%3D%221%22%2F%3E%3C%2Fsvg%3E';

        const visibleEntities = entities.map((entity: any) => ({
            entity,
            options: { 
                type: "point" as const, 
                color: "#ffffff", // Prevents Cesium from applying the default Cyan tint
                iconUrl: entity.type === 'flight' ? FLIGHT_ICON : SHIP_ICON,
                iconScale: 0.8
            }
        }));
        
        animatablesRef.current = await renderEntitiesChunked(viewer, visibleEntities, existingMap);
    });

    const updateLoop = createUpdateLoop(viewer, animatablesRef, hoveredEntityIdRef as any);
    removePostUpdateListener = viewer.scene.postUpdate.addEventListener(updateLoop);
}

export function stopAnimationLoop() {
    if (removePostUpdateListener) {
        removePostUpdateListener();
        removePostUpdateListener = undefined;
    }
    if (unsubscribeDataBus) {
        unsubscribeDataBus();
        unsubscribeDataBus = undefined;
    }
}
